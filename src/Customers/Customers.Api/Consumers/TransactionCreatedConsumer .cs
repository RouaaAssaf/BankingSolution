...
namespace Customers.Api.Consumers
{
    public class TransactionCreatedConsumer : BackgroundService
    {
        ...
        
        {
            ...
            if (evt.TransactionType == "Credit")
            {
                customer.Balance += evt.Amount;
                _logger.LogInformation("Added {Amount} to Customer {CustomerId} balance.", evt.Amount, customer.Id);
            }
            else if (evt.TransactionType == "Debit")
            {
                customer.Balance -= evt.Amount;
                _logger.LogInformation("Subtracted {Amount} from Customer {CustomerId} balance.", evt.Amount, customer.Id);
            }
